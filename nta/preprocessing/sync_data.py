...
def trim_to_session_start(beh_timeseries: pd.DataFrame) -> pd.DataFrame:
    ...
    if beh_timeseries['iOccurrence'][0]>1: # just in case it doesn't already start at 0
...
def handshake_behav_recording_sys(photo_timeseries: pd.DataFrame) -> pd.DataFrame:
    ...
    first_pulse_to_behavior = ts_.loc[ts_['toBehSys']==1].index[0]
...
def calculate_trial_length(ts: pd.DataFrame, trial_starts: list) -> list:
    ...
    # trial_lengths.append(len(ts_)-trial_starts[-1]) # last trial for length
...
def bins_per_trial_photo(photo_timeseries) -> tuple:
    ...
    trial_starts=np.insert(trial_starts, 0,0) # see handshake func
...
def sliding_corr(list1: list, list2: list, offset_range: int=30) -> tuple:
    ...
    assert max_corr_trimmed_lists > 0.99999
...
def resample_and_align(beh_timeseries: pd.DataFrame,
                       ...
                       channels: list=['grnR', 'redR', 'grnL', 'redL'],
                       by_trial: bool=False, 
                       **kwargs) -> pd.DataFrame:
    ...
    if by_trial: raise NotImplementedError
